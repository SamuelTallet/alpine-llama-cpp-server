...
# Using --no-cache to avoid caching the Alpine packages index and --virtual to group build dependencies for easier cleanup.
RUN apk add --no-cache --virtual .build-deps git g++ make cmake linux-headers curl-dev && \
...
RUN apk add --no-cache libstdc++ libcurl libgomp
